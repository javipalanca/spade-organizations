    ...
    class OwnerInformBehaviour(CyclicBehaviour):
        ...
        async def run(self):
            ...
            if (self.agent.organization.need_updates):
                ...
                msg_to_broadcast.body = json.dumps({"members": self.agent.organization.members, "rules": self.agent.organization.rules}, cls=MyEncoder)                   
                ...
                if(("org_action", "join") in msg.metadata.items()):
                    ...
                    if(self.agent.check_rules(msg)):
                        if(("org_action", "add_rule") in msg.metadata.items()):
                            ...
                            print("Attention!: The owner is unable to process the message {} because he cannot recognize the metadata".format(msg))
                            return
                    else: print("Attention!: Agent {} request '{}' failed because there is a rule that forbids the action".format(msg.sender, msg.metadata["org_action"]))
                else: print("Attention!: To request the owner you, {}, MUST be a member of the organization".format(msg.sender))